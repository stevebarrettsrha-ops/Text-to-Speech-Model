...
1. **`web/index.html` stays one file with no build step.** The CSS is the
   original Script Builder stylesheet, unchanged; new rules are appended in a
   clearly marked block. Keep the class names — the layout depends on them.
